    ...
    <div class="row">
      <div class="nine columns" style="margin-bottom: 10px">
        <ul id="menu-header" class="nav-bar horizontal">
          ...
          <li style="padding : 30px; border : 2px;"><a href="CustProd.htm"><h style= "font-size : 30px; ">Products</h></a></li>
...
<table border="1" cellpadding="5" cellspacing="5">
            ...
            <c:forEach var="pr" items="${cartitem}">
                <tr>
                <td>${pr.cartId}</td>
                    <td>${pr.quantity}</td>
                    <td>${pr.product.prodID}</td>
                    <td>${pr.price}</td>
                     <td>${pr.shoppingcart}</td>
                      <td>${pr.total}</td>
                      <td> <a href="deleteFromCart.htm?cartId=${pr.cartId}">Delete</a></td>
                    <td>  <a href="Checkout.htm?orderStatus=${pr.orderStatus}&total=${pr.total}&cartId=${pr.cartId}&prodID=${pr.product.prodID}&quantity=${pr.quantity}">Payment</a></td>
